...
class BombExplosionSystem(System):
    ...
    def update(self, bomb: Bomb, body: RigidBody, lifetime: Lifetime) -> None:
        now = time.time()

        if not bomb.has_explode and now > lifetime.dead_time:
            ...
            for direction in directions:
                for i in range(1, bomb.explosion_size + 1):
                    ...
                    if chain is not None:
                        chain_life: Lifetime = chain.get(Lifetime)
                        chain_life.expire()
                        self.update(chain.get(Bomb), chain.get(RigidBody), chain_life)
                        break
